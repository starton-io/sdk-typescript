/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import * as shared from "../shared";
import { AxiosResponse } from "axios";

export class GetBalanceAddressRequest extends SpeakeasyBase {
    /**
     * Your address.
     */
    @SpeakeasyMetadata({ data: "pathParam, style=simple;explode=false;name=address" })
    address: string;

    /**
     * The blockchain network you are checking data on.
     */
    @SpeakeasyMetadata({ data: "pathParam, style=simple;explode=false;name=network" })
    network: string;
}

export class GetBalanceAddressResponse extends SpeakeasyBase {
    @SpeakeasyMetadata()
    addressNativeBalanceResponse?: shared.AddressNativeBalanceResponse;

    /**
     * HTTP response content type for this operation
     */
    @SpeakeasyMetadata()
    contentType: string;

    /**
     * HTTP response status code for this operation
     */
    @SpeakeasyMetadata()
    statusCode: number;

    /**
     * Raw HTTP response; suitable for custom response parsing
     */
    @SpeakeasyMetadata()
    rawResponse?: AxiosResponse;
}
